...
class SectionDivider (L): # section: '<---+>'
    def __str__(self, indent = 0):
        return "%s: ---" % (super(L, self).__str__(indent))
...
class Parser(object):
    ...
    def parse_SectionDivider(self):
        self.shift()
        self.reduce(P, 1, None)
    ...
    def parse_DIV_helper(self, indent):
        if self.lookahead.isa(SL, indent):
            ...
            self.shift()
        elif self.lookahead.isa(SectionDivider):
            self.parse_SectionDivider()
        else:
            return False
