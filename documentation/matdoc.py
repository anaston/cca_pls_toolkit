...
# adapted: Nils R Winter, nils.r.winter@gmail.com
...
def render_SectionDivider():
    print "---"

def render_DIVL(tree, context):
    for n in tree.children:
        if n.isa(P):
            if isinstance(n.children[0], SectionDivider):
                render_SectionDivider()
            else:
                render_P(n, context)
...
def render(func, brief, tree, hlevel):
    print '<span style="font-size:2em;">__%s__</span>\n' % (func)
